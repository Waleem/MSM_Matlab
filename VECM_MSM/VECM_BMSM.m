function [out,L,states,diagnostics] = VECM_BMSM(r,S,F,kbar,n,startingvals, options)
    % -------------------------------------------------------------------------
    %         Vector Error Correction Bivariate Markov Switching Multifractal (VECM-BMSM)                   
    ...
    %--------------------------------------------------------------------------
    %--------------------------------------------------------------------------
    % Estimates Vector Error Correction Bivariate MSM, with the assumption that rho_m 
    % (the correlation between M^alpha and M^beta) is 1.
    ...
    se = VECM_BMSM_std_err(r, S, F, [para1 para2],kbar,n);
    ...
    if nargout>2
       [~, LLs1, LLs2, pi_mat1, pi_mat2, A1] = VECM_BMSM_likelihood1(para1, kbar, r, S, F, A_template, n);
