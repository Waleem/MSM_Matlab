function [LL,LLs] = BMSM_2stage_likelihood2(para,kbar,data,para1,n)
    % Likelihood calculation - Computes the second stage log-likelihood for the 2-stage bivariate MSM.
    % This routine does not save the state probability matrix and 
    % the transition matrices due to memory constraint. 
    % The advantage is that it is fast and can take any number of volatility components.
    
    % This code benefits a great deal, from the KronProd toolbox of Matt J. 
    % The KronProd toolbox makes it possible to construct the transition matrix
    ...
    % The BMSM_2stage_likelihood2_slow function does exactly what this function does.
    % It also saves the filtered probabilities and the transition matrix.
    % But for kbar>5, you will most likely get an out-of-memory error.
    ...
    if rho_e >=1
       rho_e=0.9999; %This has nothing to do with the estimation algorithm itself, as the condition will always be satisfied.
                       ...
                       % the objective function  at a point where rho_e >1, which violates the MSM model constraint that 
                       %rho_e =[0,1].  That is when these lines of code kick in to stop such violation.
    end
    if lamda>=1
       lamda=0.9999; 
    end
    ...
    %Get kronecker product of all the transition matrices, using the KronProd toolbox.
